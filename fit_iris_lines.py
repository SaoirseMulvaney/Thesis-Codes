...
# Function to use IRIS file to make SunPy map of fitted variable
    def mk_iris_map(self, data, header, main_header, fulldisk=False):
...
# Main function to fit IRIS raster and produce maps of intensity, Doppler velocity, and nonthermal velocity
    def fit_iris_data(self, fulldisk=False, v_nontherm=False):
        ...
        if self.window =="Si IV 1394 1403":
    ...
    # Get the line list and match to the chosen line
        ...
        # Get the line list and match to the chosen line
            main_header = extract_irisL2data.only_header(self.filename,verbose=False)
        ...
        int_map = self.mk_iris_map(intensity, header, main_header, fulldisk=fulldisk)
        ...
        doppler_map = self.mk_iris_map(v_dopp, header, main_header, fulldisk=fulldisk)
        ...
        width_map = self.mk_iris_map(line_width, header, main_header, fulldisk=fulldisk)   

        if v_nontherm:
            fwhm = 2.355 * line_width
            inst_wid = np.full((y_size, x_size),inst_ref_wid)
            v_nt = self.iris_vnt('Si', 'iv', ref_wvl, fwhm, inst_wid)
            vnt_map = self.mk_iris_map(v_nt, header, main_header, fulldisk=fulldisk)
        else:
            vnt_map = width_map
        ...
        asym_map = self.mk_iris_map(bw_asym, header, main_header, fulldisk=fulldisk)
